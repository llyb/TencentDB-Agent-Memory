...
export function renderKnowledgeToolsBlock(
  ...
  return [
    "<knowledge_tools>",
    "Use these resources only after <asset_tool_routing> selects knowledge.",
    "code-graph: require match to the current git remote/repository, then use for cross-file architecture, symbols, calls, dependencies, or impact. Use local files for exact/current code and uncommitted changes.",
    "wiki: use about to select design rationale, team definitions, historical decisions, or documented pitfalls. It does not require repository matching.",
    "Bound resources:",
    resourceTags,
    "",
    "Protocol: for a selected resource call <url>/tools/list once per session, then call <url>/tools/call using the returned exact tool name and JSON params.",
    "knowledge_id belongs in the JSON body, never in the URL. Every request needs content-type and x-tdai-service-id.",
    "List:",
    ...
    "  -H 'content-type: application/json' \\",
    ...requestHeaderLines,
    "  -d '{\"knowledge_id\":\"<id>\", \"tool_name\":\"<listed name>\", \"params\":{}}'",
    "code=0 means success. Different resources may run in parallel; after two failures, fall back to local evidence.",
    "</knowledge_tools>\n",
  ].join("\n");
}
