...
/**
 * DeepSeek's Anthropic endpoint rejects thinking-enabled requests when an
 * earlier assistant tool-use message has no signed thinking block. Session
 * init forms are generated locally by the proxy, so they can never carry an
 * upstream signature. Disable thinking for this request instead of forwarding
 * an impossible-to-replay history.
 */
export function disableThinkingForUnsignedToolHistory(
  body: Record<string, unknown>,
): { body: Record<string, unknown>; disabled: boolean } {
  const thinking = body.thinking as { type?: unknown } | undefined;
  const messages = body.messages;
  if (!Array.isArray(messages)) return { body, disabled: false };

  const hasUnsignedToolHistory = messages.some((msg) => {
    const m = msg as Record<string, unknown>;
    ...
    return !content.some((block) =>
      ...
      && hasValidThinkingSignature(block)
    );
  });

  if (!hasUnsignedToolHistory) return { body, disabled: false };
  if (thinking?.type === "disabled") return { body, disabled: false };
  return {
...
): { body: Record<string, unknown>; sanitizedCount: number; thinkingDisabled: boolean } {
  let result = body;
  if (target.bodyOverrides) {
    result = { ...result, ...target.bodyOverrides };
  }
  const sanitized = sanitizeThinkingBlocks(result);
  const compatible = disableThinkingForUnsignedToolHistory(sanitized.body);
  return {
    body: compatible.body,
    sanitizedCount: sanitized.removed,
    thinkingDisabled: compatible.disabled,
  };
}
...
// ── Session key: prefer conversation header, fallback to agent profile ───────────
  ...
  const { body: upstreamBody, sanitizedCount, thinkingDisabled } = buildUpstreamBody(body, target);
  if (sanitizedCount > 0) {
    pipe.info(
      "FORWARD",
  ...
  if (thinkingDisabled) {
    pipe.info(
      "FORWARD",
      "disabled thinking because assistant tool history has no valid upstream signature",
    );
  }
  ...
  const retryBody = disableThinkingForUnsignedToolHistory(sanitizeThinkingBlocks(body).body).body;
